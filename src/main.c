...
void set_directory(const char *id, const char *req, void *arg);
...
int main()
{
    ...
    webview_set_size(w, 600, 380, WEBVIEW_HINT_FIXED); // 750, 475
    webview_set_title(w, "Simple XOR Encryption");
    webview_bind(w, "call_encrypt", call_encrypt, NULL);
    webview_bind(w, "call_decrypt", call_decrypt, NULL);
    webview_bind(w, "set_directory", set_directory, NULL);
